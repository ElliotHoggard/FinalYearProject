...
def create_buttons1(root, parent_frame, buttons_list, column_num=1):
    ...
    for i in range(len(buttons_list)):
        if column_num == 2:
            ...
            buttons.append(Button(parent_frame, text=(buttons_list[i]["name"]), fg="White", bg="Red", font=('Autobus Bold', 12),
                                  relief=GROOVE, command=partial(button_click, buttons_list, i)))
...
# GW 32
def runEveTot1():
    from aaa.model1GW32 import EveTot1
    EveTot1()
...
def HomeAdvantage():
    ...
    root_frame = Frame(root, bg='#4c8a27')
    root_frame.place(relx=0.5, rely=0.0, relheight=1, relwidth=1, anchor='n')
    Label(root_frame, text="Measuring Home Advantage", font="AutobusBold 15 bold", bg="#add8e6").place(x=300, y=20, width=599)
...
def StatAnalysis():
    ...
    root_frame = Frame(root, bg='#4c8a27')
    root_frame.place(relx=0.5, rely=0.0, relheight=1, relwidth=1, anchor='n')
    Label(root_frame, text="Statistical Analysis", font="AutobusBold 15 bold", bg="#add8e6").place(x=300, y=20, width=599)
...
def SelectGameweek():
    ...
    buttons = [{"name": "Gameweek 30", "func_name": "Gameweek30"},
               ...
               {"name": "Gameweek 33", "func_name": "Gameweek30"}]
...
def ELOModel():
    ...
    root_frame = Frame(root, bg='#4c8a27')
    root_frame.place(relx=0.5, rely=0.0, relheight=1, relwidth=1, anchor='n')
    Label(root_frame, text="Uses ELO data to predict the winning team", font="AutobusBold 15 bold", bg="#add8e6").place(x=300, y=20, width=599)
    buttons = []
    create_buttons1(root, root_frame, buttons, 1)
    create_back_button(root)
...
def Main():
    ...
    Button(lower_frame2, text="Features Used", state="normal", fg='black',bg = "#add8e6", anchor=CENTER, command=labelactive).pack(side=BOTTOM)
